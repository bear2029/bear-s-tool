...
var NameCombo = Backbone.Collection.extend({ /*{{{*/
	...
	chi: function() {
		return this.reduce(function(str, item) {
			str.push(item.get('chi'));
			return str;
		}, []);
	},
	eng: function() {
		return this.reduce(function(str, item) {
			str.push(item.get('eng'));
			return str;
		}, []);
	},
...
var CharCollection = Backbone.Collection.extend({ /*{{{*/
	...
	randomize: function(len, pronouncePattern) {
		...
		do {
			...
			for (var i = 0; i < len; i++) {
				var sample = pronouncePattern ? new CharCollection(this.where({
					eng: pronouncePattern[i]
				})) : this;
...
var FavItemView = Backbone.View.extend({ /*{{{*/
	...
	initialize: function() {
		...
		this.attributes.parentEl.append(this.$el);
		this.dictionary = new DictionaryView({
			attributes: {
				parentEl: this.$el,
				str: this.model.chi()
			}
...
var NameGenView = Backbone.View.extend({
	...
	events: {},
	onCustom: function() {
		this.$el.addClass('custom-edting');
	},
...
var nameGenView = new NameGenView({
